...
Screenshot Directory | GFXRECON_SCREENSHOT_DIR | STRING | Direct3D 12 only option. Directory to write screenshots. Default is: Empty string (the current working directory).
Screenshot Specific Frames | GFXRECON_SCREENSHOT_FRAMES | STRING | Direct3D 12 only option. Specify one or more comma-separated frame ranges to screenshot during capture. Ranges use the same syntax and rules as `GFXRECON_CAPTURE_FRAMES`. Default is: Empty string (no scheduled screenshots).
Screenshot Interval | GFXRECON_SCREENSHOT_INTERVAL | UINT | Direct3D 12 only option. Within the ranges specified by `GFXRECON_SCREENSHOT_FRAMES`, take a screenshot every Nth frame. Default is: `1` (every frame in the range(s)).
Screenshot Format | GFXRECON_SCREENSHOT_FORMAT | STRING | Direct3D 12 only option. Image format for capture-time screenshots. Valid values are `bmp` and `png`. Default is: `bmp`
Screenshot Hotkey Trigger | GFXRECON_SCREENSHOT_TRIGGER | STRING | Direct3D 12 only option. Specify a hotkey (any one of F1-F12, TAB, ControlLeft, ControlRight) that takes a screenshot of the current frame each time it is pressed. The key must differ from `GFXRECON_CAPTURE_TRIGGER` or it will be ignored. Default is: Empty string (screenshot hotkey is disabled).
