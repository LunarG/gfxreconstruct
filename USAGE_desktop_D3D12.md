...
Enable Pipeline Library Support   | GFXRECON_ENABLE_PIPELINE_LIBRARY       | BOOL   | Direct3D 12 only option. Enables capture of ID3D12PipelineLibrary operations, allowing pipeline state objects to be loaded from a cache. When false, ID3D12PipelineLibrary operations will effectively be disabled by an override that returns an error code, forcing the caller to fallback to pipeline creation. Default is `false` 
