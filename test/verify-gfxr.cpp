...
{
    std::filesystem::path abs_app_run_directory{ APP_PATH_BASE };
    abs_app_run_directory.append(app_run_directory);

    std::filesystem::path abs_app_path{ abs_app_run_directory };
    abs_app_path.append(app_path);
    ...
    app_command += " & cd ";
    app_command += abs_app_run_directory.string();
    app_command += " & ";
    app_command += abs_app_path.string();
    auto app_result = std::system(app_command.c_str());
    ASSERT_EQ(app_result, 0) << "command failed " << app_command;

    std::filesystem::path capture_path{ abs_app_run_directory };
