...
# Capture and Replay Using the GFXReconstruct Tools
This document describes the GFXReconstruct tools which are used for capturing and replaying Vulkan API calls.

## GFXReconstruct Layer

The GFXReconstruct Layer is a Vulkan layer that captures Vulkan API calls and logs them to a GFXReconstruct capture file.
...
there is no need to set VK_LAYER_PATH - the installation process
will have set up the GFXReconstruct capture layer so that the Vulkan loader can find and load it.
...
### Linux
If you are not using GFXReconstruct from the Vulkan SDK or a Linux package, use the
following as an example of how to update the Linux `VK_LAYER_PATH`
environment variable for the GFXReconstruct capture layer. The example uses
...
The `VK_INSTANCE_LAYERS` environment variable may be used so that the the GFXReconstruct layer
is loaded by the Vulkan loader.
...
## gfxrecon-capture.py
The gfxrecon-capture.py tool is a convenience script that can be used to start
a capture and specify the capture options using a single command.
```
...
```
Most of the options for gfxrecon-capture.py result in the script setting the appropriate
capture layer environment variable, then invoking the program to be captured. Environment
variables not set by gfxrecon-capture.py can be set and exported before running gfxrecon-capture.py,
and they will be passed on as is to the capture layer.

gfxrecon-capture.py is a Python3 script. In order to use it, the Python3 interpreter
must be available. Depending on the command interpreter you are using on your system,
you may need to run the script by specifying the script as an argument to the python3
command. For example:
```
python3 gfxrecon-capture.py -o vkcube.gfxr vkcube --c 100
```
## gfxrecon-replay
...
## gfxrecon-compress
The gfxrecon-compress tool compresses or decompresses GFXReconstruct capture files. It
can also be used to change the compression format used in a capture file.
```
...
```
## gfxrecon-extract
The gfxrecon-extract tool extracts all shaders in a GFXReconstruct capture file. The extracted
shaders are placed into a specified directory.
```
...
```
## gfxrecon.py
The gfxrecon.py tool is a utility that can be used to launch all the
GFXReconstrcut commands.
```
...
optional arguments:
  -h, --help  show this help message and exit
```
gfxrecon.py is a Python3 script. In order to use it, the Python3 interpreter
must be available. Depending on the command interpreter you are using on your system,
you may need to run the script by specifying the script as an argument to the python3
command. For example:
```
python3 gfxrecon.py capture -o vkcube.gfxr vkcube --c 100
```

## Capturing on Android Devices

The GFXReconstruct Layer can be used to capture Vulkan API calls on Android devices.
...
```
gfxrecon.py install replay-debug.apk
```

### The Androld Replay Command
