...
class DecodePNextStructGeneratorOptions(GeneratorOptions):
...
# genCmd(cmdinfo)
class DecodePNextStructOutputGenerator(OutputGenerator):
    ...
    def __init__(self,
        ...
        # Typenames
        self.structureTypes = dict()                             # Map struct type to VkStructureType value
    #
    def beginFile(self, genOpts):
        ...
        self.newline()
        write('    if ((parameter_buffer != nullptr) && (buffer_size >= sizeof(VkStructureType)))', file=self.outFile)
        write('    {', file=self.outFile)
    ...
    def beginFeature(self, interface, emit):
        ...
        self.structureTypes = dict()
        self.featureExtraProtect = GetFeatureProtect(interface)
    def endFeature(self):
        # C-specific
        # Actually write the interface to the output file.
        if (self.emit) and self.structureTypes:
            ...
            for name in self.structureTypes:
                write('        case {}:'.format(self.structureTypes[name]), file=self.outFile)
                write('            (*pNext) = std::make_unique<PNextTypedNode<Decoded_{}>>();'.format(name), file=self.outFile)
                write('            bytes_read = (*pNext)->Decode(parameter_buffer, buffer_size);'.format(name), file=self.outFile)
    ...
    def genType(self, typeinfo, name, alias):
        ...
        if (category == 'struct' or category == 'union'):
            self.genStructureTypeEntry(typeinfo, name)
    ...
    def genStructureTypeEntry(self, typeinfo, typename):
        stype = None
        ...
        for member in members:
            ...
            if membername == 'sType':
                ...
                break
        if stype:
            self.structureTypes[typename] = stype
