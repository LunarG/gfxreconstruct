...
class APICallDecodeCasesGeneratorOptions(GeneratorOptions):
...
# genCmd(cmdinfo)
class APICallDecodeCasesOutputGenerator(OutputGenerator):
    ...
    def endFeature(self):
        ...
        if (self.emit and self.sections['command']):
            ...
            write(''.join(self.sections['command']), end='', file=self.outFile)
            if (self.genOpts.protectProto):
                write('#endif', file=self.outFile)
            if (self.featureExtraProtect != None):
