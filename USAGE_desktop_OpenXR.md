...
#### Vulkan Calls From Runtime Threads

An OpenXR runtime makes its own Vulkan calls when it composes and presents a frame.
The runtime uses the same `VkInstance` and `VkDevice` as the application.
As a result, these calls pass through the GFXReconstruct Vulkan layer.

The capture layer does not record the Vulkan calls that the runtime makes on the thread that called
the OpenXR entry point.
Replay reproduces those calls from the OpenXR call.
Calls from other application threads are not affected.

The capture layer does record Vulkan calls that the runtime makes from a thread of its own.
The layer cannot tell these calls apart from the calls of the application.
These calls can cause errors at replay.
The `GFXRECON_SKIP_THREADS_WITH_INVALID_DATA` option can detect some of these threads and skip
their calls.
See the capture options in [GfxReconstruct for Desktop Vulkan](./USAGE_desktop_Vulkan.md).
