...
|  Capture Dump Android Assets | debug.gfxrecon.capture_android_dump_assets | BOOL | false | Setting this triggers a dump of all assets into the asset file. Since Android options cannot be set by the layer, dumping is done whenever this option switches between from `false` to `true` or from `true` to `false`. [Deprecated, please use 'capture_dynamic_trigger_dump_assets'] |
|  Capture Trigger For Android | debug.gfxrecon.capture_android_trigger | BOOL | false | Set during runtime to `true` to start capturing and to `false` to stop. If not set at all then it is disabled (non-trimmed capture). [Deprecated, please use 'capture_dynamic_trigger'] |
