...
##### 5. `hashing`
This mode uses no shadow memory, memory protection, or signal handlers. It hashes
the memory returned by the driver from `vkMapMemory` in fixed-size blocks (using
`XXH3`) and, on `vkUnmapMemory` and `vkQueueSubmit`, re-hashes each block and
writes out the ones whose hash changed. It works the same on Linux and Android
and does not conflict with application `SIGSEGV` handlers, at the cost of
re-reading and re-hashing the whole mapped range on every flush.
...
| Capture Trigger Boundary                       | GFXRECON_CAPTURE_TRIGGER_BOUNDARY                       | STRING  | Select the boundary where GFXReconstruct checks for capture-trigger state changes. Valid values are `frames` and `queue_submits`. Use `queue_submits` for workloads without frame boundaries, such as pure compute. This setting applies only to trigger-based capture; explicit trim ranges use their own boundary. Default is: `frames`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Capture Trigger Boundary Limit                 | GFXRECON_CAPTURE_TRIGGER_FRAMES                         | STRING  | Specify a limit on the number of capture boundaries to record after the trigger starts capture. Boundaries are frames by default and queue submits when `GFXRECON_CAPTURE_TRIGGER_BOUNDARY` is `queue_submits`. Default is: Empty string (no limit)                                                                                                                                                                                                           |
...
usage: gfxrecon-capture-vulkan.py [-h]
                                  ...
                                  [--log-timestamps]
                                  [--log-file <file>]
                                  [--memory-tracking-mode {page_guard,hashing,assisted,unassisted}]
                                  <program> [<programArgs>]
...
optional arguments:
  ...
  --memory-tracking-mode {page_guard,assisted,unassisted}
                        Method to use to track changes to memory mapped objects:
                           ...
                           hasing:     Check for changes by hashing the memory
