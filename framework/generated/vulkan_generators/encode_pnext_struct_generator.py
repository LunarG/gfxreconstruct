...
class EncodePNextStructGenerator(BaseGenerator):
    ...
    def beginFile(self, genOpts):
        ...
        write('            {', file=self.outFile)
        write('                // pNext is unrecongized.  Write warning message and encode it as a NULL pointer.', file=self.outFile)
        write('                int32_t message_size = std::snprintf(nullptr, 0, "A pNext value with unrecognized VkStructureType = %x was omitted from the capture file, which may cause replay to fail.", header->sType);', file=self.outFile)
        write('                std::unique_ptr<char[]> message = std::make_unique<char[]>(message_size + 1); // Add 1 for null-terminator.', file=self.outFile)
        write('                std::snprintf(message.get(), (message_size + 1), "A pNext value with unrecognized VkStructureType = %x was omitted from the capture file, which may cause replay to fail.", header->sType);', file=self.outFile)
