...
def CreateReplayParser():
    ...
    parser.add_argument('-m', '--memory-translation', metavar='<mode>', choices=['none', 'remap', 'rebind'], help='Enable memory translation for replay on GPUs with memory types that are not compatible with the capture GPU\'s memory types.  Available modes are: none, remap, rebind (forwarded to replay tool)')
