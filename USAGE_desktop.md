...
Accerlation Struct Size Padding | GFXRECON_ACCEL_STRUCT_PADDING | FLOAT | Direct3D 12 only option. Increase the required acceleration structure size that is reported to the application by calls to `GetRaytracingAccelerationStructurePrebuildInfo`. This can enable replay in environments with increased acceleration structure size requirements. The resulting padded size is `ceil(original_size * (1.0 + value))`. Default is `0.0`
...
The text output is formatted as JSON and written to to a .txt file in the directory of the specified GFXReconstruct capture file.
