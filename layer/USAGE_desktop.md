...
Hotkey Trim Trigger | GFXRECON_TRIM_TRIGGER | STRING | Specify a hotkey (any one of F1-F12|TAB|CONTROL) that will be used to start/stop trim.  Example: `F3` will enable trim trigger start/stop capture by pressing the hotkey F3.  Default is: Empty string (hotkey trim disabled).
